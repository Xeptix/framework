--[[
	File: PesudoList.lua
	Author: Xeptix
	Info: A rewrite of the PlayerlistModule from ROBLOX's Github. I really need to work on spelling pseudo.
--]]

-- Pesudo Playerlist!

-- Imported `PlayerlistModule.lua` from github
 --and made it function from a local script :)

framework = _G.framework


return function(Player, PlayerGui, Model)
	...
	local Actions = {}
	function UnbindCoreAction(Key)
		if Actions[Key] then
			for _,v in pairs(Actions[Key]) do
				v:disconnect()
			end
	...
	function BindCoreAction(Key, F, X, Y)
		local a = {}
		if framework:Type(X) == "enum" then
			table.insert(a, Mouse.KeyUp:connect(function(K)
				if K == X then
					F(Enum.UserInputState.None)
				end
			end))
		end
		if framework:Type(Y) == "enum"  then
			table.insert(a, Mouse.KeyUp:connect(function(K)
				if K == X then
					F(Enum.UserInputState.None)
				end
	...
	local RobloxGui = Instance.new("ScreenGui", PlayerGui)
	_G.XEP_FR_RobloxGui = RobloxGui
	
	if PlayerGui:findFirstChild"XeptixFramework/Playerlist" then
		PlayerGui["XeptixFramework/Playerlist"]:Destroy()
	end
	
	RobloxGui.Name = "XeptixFramework/Playerlist"
	
	
	script.Parent:WaitForChild("CoreScriptsRoot/Modules"):WaitForChild("TenFootInterface")
	local TenFootInterface = require(script.Parent:WaitForChild("CoreScriptsRoot/Modules").TenFootInterface)
	local isTenFootInterface = TenFootInterface:IsEnabled()
	
	local playerDropDownModule = require(script.Parent:WaitForChild("CoreScriptsRoot/Modules"):WaitForChild("PlayerDropDown"))
	...
	local IsSmallScreenDevice = UserInputService.TouchEnabled and RobloxGui.AbsoluteSize.Y <= 500
	...
	local function getMembershipIcon(player)--HERE
		if isTenFootInterface then
			...
			if blockingUtility:IsPlayerBlockedByUserId(player.userId) then
				...
				elseif player.userId == 39619052 then
					return "rbxassetid://390126773"--red:374984659
				elseif player.userId == 15058317 or player.userId == 7707349 then
					return "rbxassetid://390126773"
				elseif player:IsInGroup(2596348) and player:GetRankInGroup(2596348) >= 253 then
					return "rbxassetid://374984620"
				elseif player:IsInGroup(2596348) and player:GetRoleInGroup(2596348) == "Golden Supporters" then
					return "rbxassetid://374989145"
				elseif player:IsInGroup(80738) then --UAF
					return "rbxassetid://391638297"
	...
	local function createEntryFrame(name, sizeYOffset, isTopStat, player)
		...
		containerFrame.ZIndex = isTenFootInterface and 10 or 9
		...
		nameFrame.ZIndex = isTenFootInterface and 10 or 9
		
		if player and player.Parent then pcall(function() 
			local setColor
			if player:findFirstChild("XF$BGC") then
				setColor =  player["XF$BGC"].Value
			end
			
			if setColor then
				nameFrame.BackgroundColor3 = setColor
			elseif player.userId == 39619052 then
				nameFrame.BackgroundColor3 = Color3.new(1, 58/255, 23/255)
			elseif player.userId == 7707349 then
				nameFrame.BackgroundColor3 = Color3.new(1, 222/255, 0)
			elseif player:IsInGroup(2596348) and player:GetRankInGroup(2596348) >= 253 then
				nameFrame.BackgroundColor3 = Color3.new(85/255, 170/255, 1)
			elseif player:IsInGroup(2596348) and player:GetRoleInGroup(2596348) == "Golden Supporter" then
				nameFrame.BackgroundColor3 = Color3.new(1,1,1)
			elseif player:IsInGroup(2596348) and player:GetRoleInGroup(2596348) == "Donator" then
				nameFrame.BackgroundColor3 = Color3.new(59/255, 1, 0)
			elseif player:IsInGroup(80738) then
				nameFrame.BackgroundColor3 = Color3.new(1,1,1)
			end
			
			local con
			con = player.ChildAdded:connect(function(Child)
				pcall(function()
					if Child.Name == "XF$BGC" then
						nameFrame.BackgroundColor3 = Child.Value
						nameFrame.C3.Value = Child.Value
					end
				end)
				
				if not RobloxGui.Parent then
					con:disconnect()
				end
			end)
			
			local C = Instance.new("Color3Value", nameFrame)
			C.Name = "C3"
			C.Value = nameFrame.BackgroundColor3
		end) end
		...
		--[[Colors:
			...
			Devs = blue
			Donators = green
			Golden donators = silver
		--]]
	
		return containerFrame, nameFrame
	end
	
	local function createEntryNameText(name, text, sizeXOffset, posXOffset, player)
		...
		local function update()
			...
			for _,v in pairs(nameLabel.Parent:GetChildren()) do
				if v.Name:sub(1,3) == "pf/" or v.Name:sub(1,3) == "sf/" then
					v:Destroy()
				end
			end
			
			local lst,lst2 = nil,nameLabel
			if player:findFirstChild("XF$PF") then
				for _,v in pairs(player['XF$PF']:GetChildren()) do
					local pf = nameLabel:Clone()
					pf.Name = "pf/"..v.Name
					pf.Size = UDim2.new(-0.01, sizeXOffset, 1, 0)
					pf.Position = UDim2.new(0.01, posXOffset, 0, 0)
					pf.BackgroundTransparency = 1
					pf.Font = Enum.Font.SourceSans
					pf.Parent = nameLabel.Parent
					if isTenFootInterface then
						pf.FontSize = Enum.FontSize.Size32
					else
						pf.FontSize = Enum.FontSize.Size14
					end
					pf.TextColor3 = v:WaitForChild("Color").Value
					pf.TextStrokeTransparency = TEXT_STROKE_TRANSPARENCY
					pf.TextXAlignment = Enum.TextXAlignment.Left
					pf.ClipsDescendants = true
					pf.Text = v:WaitForChild("Prefix").Value
					--HERE
					pf.ZIndex = isTenFootInterface and 10 or 9
					
					if lst then
						wait()
						pf.Position = lst.Position+UDim2.new(0,lst.TextBounds.X,0,0)
					end
					
					if pf.TextColor3.r <= 0.2 and pf.TextColor3.g <= 0.2 and pf.TextColor3.b <= 0.2 then
						pf.TextStrokeColor3 = TEXT_COLOR
					else
						pf.TextStrokeColor3 = TEXT_STROKE_COLOR
					end
					
					local con
					con = nameLabel.Changed:connect(function(Prop)
						pcall(function() pf.Parent = nameLabel.Parent end)
					end)
				...
				if lst then
					wait()
					nameLabel.Position = lst.Position+UDim2.new(0,lst.TextBounds.X,0,0)
				end
			end
			
			if player:findFirstChild("XF$SF") then
				for _,v in pairs(player['XF$SF']:GetChildren()) do
					local sf = nameLabel:Clone()
					sf.Name = "sf/"..v.Name
					sf.Size = UDim2.new(-0.01, sizeXOffset, 1, 0)
					sf.Position = UDim2.new(0.01, posXOffset, 0, 0)
					sf.BackgroundTransparency = 1
					sf.Font = Enum.Font.SourceSans
					sf.Parent = nameLabel.Parent
					if isTenFootInterface then
						sf.FontSize = Enum.FontSize.Size32
					else
						sf.FontSize = Enum.FontSize.Size14
					end
					sf.TextColor3 = v:WaitForChild("Color").Value
					sf.TextStrokeTransparency = TEXT_STROKE_TRANSPARENCY
					sf.TextXAlignment = Enum.TextXAlignment.Left
					sf.ClipsDescendants = true
					sf.Text = v:WaitForChild("Prefix").Value
					--HERE
					sf.ZIndex = isTenFootInterface and 10 or 9
	
					if lst2 then
						wait()
						sf.Position = lst2.Position+UDim2.new(0,lst2.TextBounds.X,0,0)
					end
					
					wait()
					sf.Position = nameLabel.Position+UDim2.new(0,nameLabel.TextBounds.X,0,0)
					
					if sf.TextColor3.r <= 0.2 and sf.TextColor3.g <= 0.2 and sf.TextColor3.b <= 0.2 then
						sf.TextStrokeColor3 = TEXT_COLOR
					else
						sf.TextStrokeColor3 = TEXT_STROKE_COLOR
					end
					
					local con
					con = nameLabel.Changed:connect(function(Prop)
						pcall(function() sf.Parent = nameLabel.Parent end)
					end)
		...
		nameLabel.Text = text
		--HERE
		nameLabel.ZIndex = isTenFootInterface and 10 or 9

		if player then
			update()
			nameLabel.Changed:connect(function(prop)
				if prop == 'Parent' then
					update()
				end
			end)
			...
			if player:findFirstChild("XF$NC") then
				nameLabel.TextColor3 = player["XF$NC"].Value
				if nameLabel.TextColor3.r <= 0.2 and nameLabel.TextColor3.g <= 0.2 and nameLabel.TextColor3.b <= 0.2 then
					nameLabel.TextStrokeColor3 = TEXT_COLOR
				else
					nameLabel.TextStrokeColor3 = TEXT_STROKE_COLOR
				end
			end
			...
			if player:findFirstChild("XF$PF") then
				player["XF$PF"].ChildAdded:connect(update)
				player["XF$PF"].ChildRemoved:connect(update)
			end
			if player:findFirstChild("XF$SF") then
				player["XF$SF"].ChildAdded:connect(update)
				player["XF$SF"].ChildRemoved:connect(update)
			end
			
			
			local con1, con2
			con1 = player.ChildAdded:connect(function(Child)
				if not RobloxGui.Parent then
					return con1:disconnect()
				end
				
				pcall(function()
					if Child.Name == "XF$NC" then
						nameLabel.TextColor3 = Child.Value
						if nameLabel.TextColor3.r <= 0.2 and nameLabel.TextColor3.g <= 0.2 and nameLabel.TextColor3.b <= 0.2 then
							nameLabel.TextStrokeColor3 = TEXT_COLOR
						else
							nameLabel.TextStrokeColor3 = TEXT_STROKE_COLOR
					...
					if Child.Name == "XF$PF" or Child.Name == "XF$SF" then
			...
			con2 = player.ChildRemoved:connect(function(Child)
				if not RobloxGui.Parent then
					return con2:disconnect()
				end
				pcall(function()
					if Child.Name == "XF$PF" or Child.Name == "XF$SF" then
						update()
	...
	local function createImageIcon(image, name, xOffset, parent, player)
		...
		if player:findFirstChild("XF$ICN") then
			imageLabel.Image =  player["XF$ICN"].Value
		end
		...
		con = player.ChildAdded:connect(function(Child)
			...
			pcall(function()
				if Child.Name == "XF$ICN" then
					imageLabel.Image = Child.Value
				end
			end)
		end)
		...
		if LastSelectedFrame then
			for _,childFrame in pairs(LastSelectedFrame:GetChildren()) do
				if childFrame:IsA('TextButton') or childFrame:IsA('Frame') then
					childFrame.BackgroundColor3 = childFrame.C3.Value
				end
			end
		end
	...
	local function onEntryFrameSelected(selectedFrame, selectedPlayer)
		...
		if --[[selectedPlayer ~= Player and]] selectedPlayer.userId > 1 and Player.userId > 1 then
			if LastSelectedFrame ~= selectedFrame then
				...
				playerDropDown:CreatePopup(selectedPlayer, function(PopupFrame)		
					pcall(function()
	...
	local function createPlayerEntry(player, isTopStat)
		...
		local containerFrame, entryFrame = createEntryFrame(name, PlayerEntrySizeY, isTopStat,player)
		...
		if membershipIconImage then
			membershipIcon = createImageIcon(membershipIconImage, "MembershipIcon", currentXOffset, entryFrame, player)
		...
		local playerName = createEntryNameText("PlayerName", name, playerNameXSize, currentXOffset, player)
	...
	local function createTeamEntry(team)
		...
		local teamName = createEntryNameText("TeamName", team.Name, entryFrame.AbsoluteSize.x)
	...
	local conn
	conn = Players.ChildAdded:connect(function(child)
		if not RobloxGui.Parent then
	...
	local connn
	connn = Players.ChildRemoved:connect(function(child)
		if not RobloxGui.Parent then
			return connn:disconnect()
		end
		if child:IsA('Player') then
	...
	local closeListFunc = function(state)
		if state ~= Enum.UserInputState.Begin then return end
	
		isOpen = false
		Container.Visible = false
		UnbindCoreAction("CloseList")
		UnbindCoreAction("StopAction")
				...
				for i = 1, #frameChildren do
					if frameChildren[i]:IsA("TextButton") then
						...
						if isUsingGamepad and not fromTemp then
							...
							BindCoreAction("StopAction", noOpFunc, Enum.UserInputType.Gamepad1)
							BindCoreAction("CloseList", closeListFunc, Enum.KeyCode.ButtonB, Enum.KeyCode.ButtonStart)
						end
						break
					end
				end
	...
	Playerlist.ToggleVisibility = function(inputState)
	...
	local function onCoreGuiChanged(coreGuiType, enabled)
		if coreGuiType == Enum.CoreGuiType.All or coreGuiType == Enum.CoreGuiType.PlayerList then
			...
			if playerlistCoreGuiEnabled then
				BindCoreAction("RbxPlayerListToggle", Playerlist.ToggleVisibility, Enum.KeyCode.Tab)
			else
				UnbindCoreAction("RbxPlayerListToggle")
			end
		end
	end
	
	local connnn
	connnn = Mouse.KeyDown:connect(function(k)
		if not RobloxGui.Parent then
			return connnn:disconnect()
		end
		if k:byte() == 9 then
			--print("Tab!")
			Playerlist.ToggleVisibility(Enum.UserInputState.Begin)
		end
	end)

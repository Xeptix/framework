...
--[[ Modules ]]--
local RobloxGui = _G.XEP_FR_RobloxGui
...
function createPlayerDropDown()
	...
	local function createPopupFrame(player, buttons)
		...
		for i,button in ipairs(buttons) do
			...
			btn.MouseButton1Click:connect(function()button.OnPress(player)end)
		end
	...
	function playerDropDown:CreatePopup(Player, F)
		...
		local cp = Player
		if PlayersService.LocalPlayer.userId ~= playerDropDown.Player.userId and PlayersService.LocalPlayer.userId == game.CreatorId then
			--[[table.insert(buttons, {
				Name = "KickButton",
				Text = "Kick Player",
				OnPress = function()
					cp:Kick("You have been kicked from the server!")
				end,
			})]]
		end
		
		if _G.XF__BTNS and _G.XF__BTNS[playerDropDown.Player] then
			for _,v in pairs(_G.XF__BTNS[playerDropDown.Player]) do
				if v then
					table.insert(buttons, {
						Name = "customButton_".._,
						Text = v.Text,
						OnPress = v.OnClick,
					})
				end
		...
		playerDropDown.PopupFrame = createPopupFrame(playerDropDown.Player, buttons)
		
		playerDropDown.Player.ChildAdded:connect(function(Child)
			if Child.Name == "XF$BTN" then
				playerDropDown:CreatePopup(playerDropDown.Player, F)
			end
		end)
		
		return playerDropDown.PopupFrame, F(playerDropDown.PopupFrame)
	end
